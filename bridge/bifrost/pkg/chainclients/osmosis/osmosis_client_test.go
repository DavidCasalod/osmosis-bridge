...
func GetMetricForTest(c *C) *metrics.Metrics {
	if m == nil {
		...
		m, err = metrics.NewMetrics(config.BifrostMetricsConfiguration{
			...
			Chains:       ConvertChains(common.Chains{common.OSMOSISChain}),
		})
		c.Assert(m, NotNil)
		c.Assert(err, IsNil)
	}
...
func (s *CosmosTestSuite) TestGetAddress(c *C) {
	...
	cc := OsmosisClient{
		cfg:           config.BifrostChainConfiguration{ChainID: ConvertChain(common.OSMOSISChain)},
...
func (s *CosmosTestSuite) TestProcessOutboundTx(c *C) {
	...
	client, err := NewOsmosisClient(s.thorKeys, config.BifrostChainConfiguration{
		ChainID: ConvertChain(common.OSMOSISChain),
		RPCHost: server.URL,
...
func (s *CosmosTestSuite) TestSign(c *C) {
	...
	clientConfig := config.BifrostChainConfiguration{ChainID: ConvertChain(common.OSMOSISChain)}
	scannerConfig := config.BifrostBlockScannerConfiguration{ChainID: ConvertChain(common.OSMOSISChain)}
	...
	meta := client.accts.Get(thorCommon.PubKey(pk))
